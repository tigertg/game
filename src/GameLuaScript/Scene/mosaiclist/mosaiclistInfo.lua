...
function MosaicInfo:new(data,x,y,parm,params)
	...
	this.bg = display.newSprite(IMG_PATH.."image/common/item_bg.png")
	...
	setAnchPos(title_box,15,17)
	this.layer:addChild(title_box)
	
	
	local head_img = display.newSprite(IMG_PATH.."image/prop/s_"..cid..".png")
	setAnchPos(head_img,2+(title_box:getContentSize().width - head_img:getContentSize().width)/2 + 10,(title_box:getContentSize().height - head_img:getContentSize().height)/2+2 + 17)
	this.layer:addChild(head_img)
	
	local name = display.strokeLabel( data["name"] , title_box:getContentSize().width + 10 + 10, title_box:getContentSize().height - 20 + 17, 16 , ccc3( 0xac , 0x25 , 0x10 )  )
	this.layer:addChild( name)
	
	local lv = display.strokeLabel( "Lv" .. data["lv"] , title_box:getContentSize().width + 120 , title_box:getContentSize().height - 20 + 17, 16 , ccc3( 0xac , 0x25 , 0x10 )  )
	this.layer:addChild( lv )
	
	local pro = display.strokeLabel( Config_Property[data["effect"]]..": "..data["figure"] , title_box:getContentSize().width + 10 + 10 , title_box:getContentSize().height - 45 + 17, 16 , ccc3( 0xac , 0x25 , 0x10 )  )
	this.layer:addChild( pro )
	
	local num = display.strokeLabel("数量：".. data["num"] , title_box:getContentSize().width + 10 + 10, title_box:getContentSize().height - 67 + 17, 16 , ccc3( 0xac , 0x25 , 0x10 )  )
	this.layer:addChild( num )
	
	--镶嵌
	local btn = KNBtn:new(COMMONPATH , {"btn_bg.png"} , 320, 18 + 17, {
